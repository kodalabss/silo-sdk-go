...
func (st *LCTState) Mix(b byte) (v1, v2, v3 uint64) {
	...
	vv1.Add(vv1, big.NewInt(19)).Mod(vv1, p)
	vv2 := new(big.Int).Mul(big.NewInt(11), x)
	vv2.Add(vv2, new(big.Int).Add(e, big.NewInt(26))).Mod(vv2, p)
	vv3 := new(big.Int).Mul(big.NewInt(17), x)
	vv3.Add(vv3, new(big.Int).Add(big.NewInt(19), new(big.Int).Mul(i, big.NewInt(2)))).Mod(vv3, p)
...
func (st *LCTState) Unmix(v1, v2, v3 uint64) byte {
	...
	rx.Add(rx, new(big.Int).Mul(adj12, vv2)).Add(rx, new(big.Int).Mul(adj13, vv3))
...
func LCTPack(data []byte, seed []byte) string {
	st := NewLCTState(seed)
	buf := make([]byte, len(data)*24) // 3 uint64s per byte
	for i, b := range data {
		v1, v2, v3 := st.Mix(b)
